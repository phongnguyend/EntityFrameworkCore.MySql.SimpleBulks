...
public class BulkUpdateBuilder<T>
{
    ...
    public BulkUpdateResult Execute(IEnumerable<T> data)
    {
        ...
        updateStatementBuilder.AppendLine($"SET {string.Join("," + Environment.NewLine, _columnNames.Select(x => CreateSetStatement(x, "a", "b")))}");
    ...
    public BulkUpdateResult SingleUpdate(T dataToUpdate)
    {
        ...
        dataToUpdate.ToMySqlParameters(propertyNamesIncludeId, valueConverters: _valueConverters).ForEach(x => updateCommand.Parameters.Add(x));
