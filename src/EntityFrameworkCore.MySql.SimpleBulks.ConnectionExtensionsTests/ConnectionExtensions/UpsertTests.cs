...
using EntityFrameworkCore.MySql.SimpleBulks.Upsert;
...
{
    public UpsertTests(ITestOutputHelper output, MySqlFixture fixture) : base(output, fixture, "SimpleBulks.BulkMerge")
    {
    }
    ...
    public void Upsert_Existing_Should_Update_If_Updated_Columns_Sepecified(int length)
    {
        ...
        var result = connectionContext.Upsert(entry,
             x => x.Id,
        ...
        configurationEntriesInDb = configurationEntriesInDb.OrderBy(x => x.Id).ToList();
    ...
    public void Upsert_NonExisting_Should_Insert_If_Inserted_Columns_Sepecified(int length)
    {
        ...
        for (int i = length; i < length * 2; i++)
        {
            ...
            var result = connectionContext.Upsert(row,
                x => x.Id,
                x => new { },
    ...
    public void Upsert_Existing_Do_Nothing(int length)
    {
        ...
        var result1 = connectionContext.Upsert(entry,
             x => x.Id,
             x => new { },
             x => new { },
             options: new BulkMergeOptions
        ...
        var result2 = connectionContext.Upsert(entry,
             x => x.Id,
             x => new { },
             x => new { x.Key, x.Value, x.Description, x.IsSensitive, x.CreatedDateTime },
             options: new BulkMergeOptions
