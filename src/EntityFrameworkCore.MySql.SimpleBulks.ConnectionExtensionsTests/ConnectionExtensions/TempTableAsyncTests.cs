...
public class TempTableAsyncTests : BaseTest
{
    ...
    [Fact]
    public async Task Non_Entity_Type_Single_Table_With_Selected_Columns()
    {
        var connectionContext = new ConnectionContext(_connection, null);

        var customers = new List<CustomerDto>();

        var tableName = await connectionContext.CreateTempTableAsync(_customers,
   x => new
   {
       x.IdNumber,
       x.FirstName,
    ...
    new TempTableOptions
    {
        ...
        connectionContext.ExecuteReader(sql, reader =>
        {
    ...
    [Fact]
    public async Task Non_Entity_Type_Multiple_Tables_With_Selected_Columns()
    {
        var connectionContext = new ConnectionContext(_connection, null);

        var result = new List<dynamic>();

        var customerTableName = await connectionContext.CreateTempTableAsync(_customers,
   x => new
   {
       x.IdNumber,
       x.FirstName,
     ...
     new TempTableOptions
     {
        ...
        var contactTableName = await connectionContext.CreateTempTableAsync(_contacts,
           x => new
           {
               x.EmailAddress,
               x.PhoneNumber,
       ...
       new TempTableOptions
       {
        ...
        Assert.Equal(result[0].EmailAddress, "chuot@gmail.com");
        Assert.Equal(result[0].PhoneNumber, "+84123456790");
        ...
        Assert.Equal(result[1].EmailAddress, "ti@gmail.com");
        Assert.Equal(result[1].PhoneNumber, "+84123456789");
    ...
    [Fact]
    public async Task Non_Entity_Type_Single_Table_With_All_Columns()
    {
        var connectionContext = new ConnectionContext(_connection, null);
    ...
    [Fact]
    public async Task Non_Entity_Type_Multiple_Tables_With_All_Columns()
    {
        var connectionContext = new ConnectionContext(_connection, null);
 ...
 new TempTableOptions
 {
        ...
        connectionContext.ExecuteReader(sql, reader =>
  {
    ...
    [Fact]
    public async Task Entity_Type_Multiple_Tables()
    {
        var connectionContext = new ConnectionContext(_context.GetMySqlConnection(), null);
        ...
        // Act
        var customerTableName = await connectionContext.CreateTempTableAsync(customers,
       ...
       new TempTableOptions
       {
        ...
        var contactTableName = await connectionContext.CreateTempTableAsync(contacts,
    ...
    [Fact]
    public async Task Entity_Type_Single_Table_With_Mappings()
    {
        var connectionContext = new ConnectionContext(_context.GetMySqlConnection(), null);
        ...
        // Act
        var tableName = await connectionContext.CreateTempTableAsync(configurationEntries,
      ["Id", "Key", "Value"],
     new TempTableOptions
     {
         LogTo = LogTo
