...
{
    public DirectDeleteAsyncTests(ITestOutputHelper output, MySqlFixture fixture) : base(output, fixture, "SimpleBulks.DirectDelete")
    {
    ...
    public async Task DirectDelete_SpecifiedKeys(int index)
    {
        ...
        var deleteResult1 = await connectionContext.DirectDeleteAsync(row, x => x.Id, options: options);

        var deleteResult2 = await connectionContext.DirectDeleteAsync(compositeKeyRow, x => new { x.Id1, x.Id2 }, options: options);
    ...
    public async Task DirectDelete_SpecifiedKeys_DynamicString(int index)
    {
        ...
        var deleteResult1 = await connectionContext.DirectDeleteAsync(row, ["Id"], options: options);

        var deleteResult2 = await connectionContext.DirectDeleteAsync(compositeKeyRow, ["Id1", "Id2"], options: options);
